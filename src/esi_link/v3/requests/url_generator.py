"""URL generator for ESI requests."""
...
class UrlGenerator(UrlGeneratorProtocol):
    def __call__(self, request: Request, schema: IndexedEsiSchema) -> GeneratedUrlInfo:
        ...
        path_template = schema.operations[request.operation_id].path
    ...
    def generate_cache_url(self, request: Request, schema: IndexedEsiSchema) -> str:
        ...
        different cache urls for requests that should share a cache key.
        """
        path_url = self.generate_path_url(request, schema)
        query_parameters = request.query_parameters or {}
        cache_url = _cache_url(path_url, query_parameters)
        return cache_url
    ...
    def generate_cache_key(self, request: Request, schema: IndexedEsiSchema) -> UUID:
        ...
        a cache key.
        """
        cache_url = self.generate_cache_url(request, schema)
        cache_key = _cache_key_from_url(cache_url)
        return cache_key
