...
class HttpResponse2:
    ...
    def to_string(self, indent: int) -> str:
        """Return a string representation of the HttpResponse with the specified indentation."""
        root_model = HttpResponseRoot2(self)
        json_str = root_model.model_dump_json(indent=indent)
        return json_str

    @classmethod
    def from_string(cls, json_str: str) -> HttpResponse2:
        """Parse the HttpResponse from a JSON string."""
        value = HttpResponseRoot2.model_validate_json(json_str).root
        return value
...
HttpResponseRoot2 = RootModel[HttpResponse2]
