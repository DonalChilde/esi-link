"""This module defines the ResponseGroupHandlerPluginLoader class, which implements the ResponseGroupHandlerPluginLoaderProtocol."""
...
from esi_link.models_and_protocols import (
    ...
    ResponseGroupHandlerPluginLoaderProtocol,
)

logger = logging.getLogger(__name__)


class ResponseGroupHandlerPluginLoader(ResponseGroupHandlerPluginLoaderProtocol):
    def __init__(self, config_path: Path):
        """Initialize the ResponseGroupHandlerPluginLoader with the given configuration path."""
    ...
    ) -> Iterable[type[ResponseGroupHandlerABC]]:
    ...
    def _load_plugin(
        ...
        # Ensure the loaded class is a subclass of ResponseGroupHandlerABC
        if not issubclass(cls, ResponseGroupHandlerABC):
            raise TypeError(
                f"{plugin_config.class_name} must subclass ResponseGroupHandlerABC"
            )
        return cls
