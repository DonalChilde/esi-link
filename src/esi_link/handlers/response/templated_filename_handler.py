...
def check_directory(output_dir: str) -> Path:
...
def stringify_template_values(values: dict[str, Any]) -> dict[str, str]:
...
def sanitize_template_value(value: str) -> str:
    ...
    cleaned = _SAFE_CHAR_RE.sub("_", ascii_only)
    cleaned = re.sub(r"_+", "_", cleaned)
    return cleaned.strip("._ ")


def sanitize_template_mapping(mapping: dict[str, str]) -> dict[str, str]:
    """Sanitize all template values before rendering.

    This prevents user-provided values from creating unexpected path segments.
    """
    return {key: sanitize_template_value(value) for key, value in mapping.items()}
...
def clip_filename(filename: str, max_len: int) -> str:
    ...
    suffix = sanitize_component(suffix)
...
def as_safe_iso(iso_value: str | None) -> str:
...
def build_template_context(response: Response) -> dict[str, str]:
    ...
    context: dict[str, str] = {
        ...
        "received_at": (
            as_safe_iso(http_response.received_at.format_iso())
            if http_response
            else "unknown"
        ),
...
def sanitize_relative_path(
    ...
    safe_parts[-1] = clip_filename(safe_parts[-1], max_filename_length)
...
class TemplatedFilenameResponseHandler(ResponseHandlerABC):
    ...
    async def __call__(self, response: Response) -> Response:
        ...
        template_context = sanitize_template_mapping(template_context)
        output_file = render_template_path(
    ...
    def from_config(cls, config: ResponseHandlerConfig) -> Self:
        ...
        try:
            output_dir = check_directory(config.config["output_dir"])
            ...
            template_values = stringify_template_values(raw_template_values)
    ...
    def validate_config(cls, config: ResponseHandlerConfig) -> None:
        ...
        cls._check_available_keys(config, expected_keys)
...
# Split out the filename and path validation functions to their own helper file.
# allow optional restriction of path  to sub paths of home direfctory for safety, and test that.
# Has to be optional to allow use of temporary directories and other non-user paths.
# add an `only_user_paths` option to the handler config that enables a check that the resolved output file is under the user's home directory, and add tests for that.
